    ...
    struct loc cursor)
{
    ...
    if (cursor.y == 0) {
         ...
         * borg to work around the flush Attempt to catch "Attempt it anyway?
         * [y/n]"
         */
        if (prefix(buf, "Atte")) {
        ...
        if (!borg_inkey(false) && strncmp(buf, "Direction", 9) == 0) {
            if (borg_confirm_target) {
                ...
                borg_note("** UNEXPECTED REQUEST FOR DIRECTION Dumping "
                          "keypress history ***");
        ...
        if (prefix(buf, "Word of Recall is already")) {
            /* this shouldn't happen but can if the borg is "playing with" and
             * unknown rod */
    ...
    if (cursor.y == 1 && !borg_inkey(false) && prefix(buf, "Which effect?")) {
        if (borg_cfg[BORG_VERBOSE])
